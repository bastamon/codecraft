...
def predict_vm(ecs_lines, input_lines):
    ...
    flavorD = []
    for i in flavorDd:
        flavorD.append(4 * int(round(i[-1])))
    print flavorD
    ...
    while flag:
        for i in range(len(pkcount) - 1, 0, - 1):
            for j in range(len(pkcount[i]) - 1 - bias, 0, - 1):
                if optp[i] != optp[i - 1] and optp[i][j] == optp[i - 1][j - pkcount[i][j] * w[i - 1]] + \
                                pkcount[i][j] * v[i - 1] and pkcount[i][j] != 0 and maxvalue - pkcount[i][j] * v[
                            i - 1] >= 0:
