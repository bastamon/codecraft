...
def predict_vm(ecs_lines, input_lines):
    ...
    flavorD = []
    for i in flavorDd:
        flavorD.append( 2* int(round(i[-1])))
    print flavorD
    ...
    for i in range(len(flavorList)):
        ...
        CflavorD.append(4*int(flavorD[i]))
    # print CflavorD
    Vir_Request, N_host, cpurate, ramrate, cpueffvir, rameffvir = assembly_result(Acpu, Bram, CflavorD, Dimop, pcpu,
                                                                                  pmem)
    # output.txt
    strline = str(C) + '\r\n'
    for i in range(len(flavorList)):
...
def multlists(list1, list2, isfloat = False):
    if len(list1) == len(list2) and list1 != [] and list2 != [] and type(list1[0]) != list and type(list2[0]) != list:
        productlist = []
        for i in range(len(list1)):
            if isfloat != True:
                productlist.append(math.ceil(1.0 * list1[i] * list2[i]))
            else:
                productlist.append(1.0 * list1[i] * list2[i])
        return productlist
    else:
        return [0, 0]
...
def reciprocated(A):
    ...
    # for i in A:
    #     a.append(1.0 / i)
    return [1.0/ i for i in A]


def substructlst(x, y):
...
def MultiplePack(n, m, wt, vt, numt, m1, dimop):
    optp = [[0 for _ in range(m + 1)] for _ in range(n + 1)]
    pkcount = [[0 for _ in range(m + 1)] for _ in range(n + 1)]
    # sorting 1:2.28
    refval = 1.0 / 2.28 if dimop == 'CPU' or dimop == 'CPU\n' else 2.28
    ratio = multlists(vt, reciprocated(wt), True)
    difflst = abslst(substructlst(ratio, [refval for _ in range(len(ratio))]))
    I, diff = index_sort(difflst)
    ...
    for i in I:
        ...
        num.append(numt[i])
        w.append(wt[i])
    # v.append(vt[i] for i in I)
    # num.append(numt[i] for i in I)
    # w.append(wt[i] for i in I)
    ...
    # regression
    virs_C = [0 for _ in range(len(virs_count))]
    for i in range(len(virs_count)):
        virs_C[I[i]] = virs_count[i]

    return max_value, virs_C
...
def exchange(optp, max_value, pkcount, w, v, pres):
    ...
    bias = 0
    cth = 1
    # for i in range(len(pkcount[0])-1,0,-1):
    #     if optp[len(optp) - 1][i] == optp[len(optp) - 1][i-1]:
    #         cth += 1
    #     else:
    #         break
    ...
    while flag:
        ...
        for i in range(len(pkcount) - 1, 0, - 1):
            for j in range(len(pkcount[i]) -1 - bias - cth, 0, - 1):
                if optp[i] != optp[i - 1] and optp[i][j] == optp[i - 1][j - pkcount[i][j] * w[i - 1]] + pkcount[i][j] * v[i - 1] and pkcount[i][j] != 0 and maxvalue - pkcount[i][j] * v[i - 1] >= 0 :
        ...
        if sum(multlists(result, w)) <= pres and sum(multlists(result, v)) <= max_value:
            if sum(multlists(result, v)) < max_value and cth != 0:
                print 'Exception:supposed to be equal to ' + str(max_value) + ' not to ' + str(
                    sum(multlists(result, v)))
                # print str(cth)
                pass

            elif sum(multlists(result, v)) == max_value or cth ==0 :
                flag = False
                # print 'Done'+str(cth)
            ...
            if sum(C) == 0:
                ...
                if sum(C) == 0:
                    ...
                    for k in range(0, len(C)):
                        C[k] = C[k] - cpuvir[k]
                costCPU, cpuvir = MultiplePack(len(C), pmen, B, A, C, pcpu, dimop)  # costCPU+56
                ...
                if sum(C) == 0:
                    ...
                    for k in range(0, len(C)):
                        C[k] -= ramvir[k]
                costRAM, ramvir = MultiplePack(len(C), pcpu, A, B, C, pmen, dimop)  # costRAM+128
