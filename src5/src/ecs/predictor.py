...
def predict_vm(ecs_lines, input_lines):
    ...
    for i in range(flavorNum):
        ...
        prameters = L_layer_model(xx, yy[i], [N, 24, 12, 1], 0.0001, 100)
...
def MultiplePack(n, m, w, v, num, m1):
    ...
    pkcount = [[0 for _ in range(m)] for _ in range(n)]
                ...
                if j >= k * w[i - 1] and optp[i][j] < optp[i - 1][j - k * w[i - 1]] + k * v[i - 1] and optp[i - 1][j - k * w[i - 1]] + k * v[i - 1] < m1:
                    ...
                    if  i >= 1 and j >= 1:
                        pkcount[i-1][j-1] = k
                                # k*w[i-1]
    max_value = optp[n][m]
    virs_count = exchange(max_value, v, num, m)
...
def exchange(max_value, pkcount, num, pres):
    result = [0 for _ in range(len(num))]
    for i in range(len(pkcount)-1,-1,-1):
        for j in range(len(pkcount[i])-1,-1,-1):
            if pkcount[i][j] != 0:
                result[i]=pkcount[i][j]
                break

    # max_value is good opt
    # pkcount is k
    # num is border of count
    # pres
...
#         xi = X[i][0]
#         x.append(x)
#         W[i,i] = math.exp(math.sqrt(pow(x-xi,2))/(-2*k**2))
#     xWx = X.T * W * X
#     if det(xWx) == 0:
