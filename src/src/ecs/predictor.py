...
# main#############print ('hello')
def assembly_result(A, B, C, pCPU, pMEM, dimop):
    Vir_Request = [sum(multlists(C, A)), sum(multlists(C, B))]
    N_host = int(math.ceil(max(multlists(Vir_Request, [1 / float(pCPU), 1 / float(pMEM)], ))))
            ...
            if C == [0 for _ in range(len(C))]:
                ...
                if dimop == 'CPU' or dimop == 'CPU\n':
                    if cpuvir != []:
                        for k in range(0, len(C)):
                            C[k] -= cpuvir[k]
                    costCPU, cpuvir = MultiplePack(len(C), pMEM, B, A, C, pCPU)  # costCPU+56
                ...
                if dimop == 'MEM' or dimop == 'MEM\n':
                    if ramvir != []:
                        for k in range(0, len(C)):
                            C[k] -= ramvir[k]
                    costRAM, ramvir = MultiplePack(len(C), pCPU, A, B, C, pMEM)  # costRAM+128
